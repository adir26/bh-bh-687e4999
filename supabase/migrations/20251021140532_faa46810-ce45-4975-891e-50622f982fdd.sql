-- Update create_order_bundle RPC to include product_id and lead_id
...
BEGIN
  ...
  IF (payload->'project'->>'mode') = 'create' THEN
    INSERT INTO public.projects (client_id, created_by, title, detailed_status, address_json)
    VALUES (
  ...
  -- ===== STEP 2: Create Order (with lead_id) =====
  ...
  -- ===== STEP 3: Create Order Items (with product_id) =====
  INSERT INTO public.order_items (order_id, product_id, product_name, description, quantity, unit_price)
  SELECT
    v_order_id,
    (item->>'product_id')::uuid,
    COALESCE(item->>'product_name', item->>'name'),
    item->>'description',
    COALESCE((item->>'quantity')::numeric, (item->>'qty')::numeric),
