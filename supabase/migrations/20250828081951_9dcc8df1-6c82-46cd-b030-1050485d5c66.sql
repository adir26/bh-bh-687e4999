-- Supplier Orders feature migration (idempotent, non-breaking)
-- 1) Add new columns to orders without altering existing status enum/type
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'orders' AND column_name = 'current_status'
  ) THEN
...
-- Backfill current_status and total_ils where missing (map from existing status/amount)
...
-- Create CHECK constraint for current_status if not exists
DO $$ BEGIN
  IF NOT EXISTS (
...
-- 3) RPC helpers with server-side validation using current_status
...
BEGIN
  ...
  IF v_current_status IS NULL THEN
    -- initialize from legacy column if needed
...
-- 4) Storage bucket and policies for order attachments
INSERT INTO storage.buckets (id, name, public)
VALUES ('orders', 'orders', false)
ON CONFLICT (id) DO NOTHING;

-- Remove previous policy if exists (name may vary) and create precise policy using order_id folder
DO $$ BEGIN
  PERFORM 1 FROM pg_policies WHERE schemaname = 'storage' AND tablename = 'objects' AND policyname = 'orders_files_supplier_access';
  IF FOUND THEN
    EXECUTE 'DROP POLICY "orders_files_supplier_access" ON storage.objects';
  END IF;
END $$;
...
CREATE POLICY IF NOT EXISTS "order_status_events_access" ON public.order_status_events
FOR ALL TO authenticated USING (public.rpc_supplier_can_access(order_id)) WITH CHECK (public.rpc_supplier_can_access(order_id));

CREATE POLICY IF NOT EXISTS "order_attachments_access" ON public.order_attachments
FOR ALL TO authenticated USING (public.rpc_supplier_can_access(order_id)) WITH CHECK (public.rpc_supplier_can_access(order_id));

CREATE POLICY IF NOT EXISTS "order_calls_access" ON public.order_calls
FOR ALL TO authenticated USING (public.rpc_supplier_can_access(order_id)) WITH CHECK (public.rpc_supplier_can_access(order_id));
