...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_created_at_btree ON public.orders(created_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_amount_btree ON public.orders(amount);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_status_created_at ON public.orders(status, created_at);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_projects_budget_range ON public.projects(budget_min, budget_max);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_projects_dates ON public.projects(start_date, end_date);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_projects_status_client ON public.projects(status, client_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_products_price_range ON public.products(price) WHERE is_published = true;
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_products_search_text ON public.products USING gin(to_tsvector('english', name || ' ' || COALESCE(description, '')));

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_quotes_dates ON public.quotes(created_at, valid_until);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_quotes_amount_status ON public.quotes(total_amount, status);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reviews_rating_created ON public.reviews(rating, created_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_reviews_reviewed_rating ON public.reviews(reviewed_id, rating);
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_notifications_priority ON public.notifications(priority, created_at);

-- Composite indexes for common queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_leads_supplier_status_date ON public.leads(supplier_id, status, expected_close_date);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_lead_activities_lead_date ON public.lead_activities(lead_id, created_at);

-- Full-text search indexes
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_companies_search_text ON public.companies USING gin(to_tsvector('english', name || ' ' || COALESCE(description, '')));
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_support_tickets_search ON public.support_tickets USING gin(to_tsvector('english', title || ' ' || description));

-- Time-series partitioning for large tables (preparation)
-- Analytics tables partitioned by month
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_user_analytics_date_metric ON public.user_analytics(metric_date, metric_name);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_company_analytics_date_metric ON public.company_analytics(metric_date, metric_name);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_search_history_date ON public.search_history(created_at);
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_audit_logs_date ON public.audit_logs(created_at);
