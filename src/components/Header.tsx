...
export const Header: React.FC<HeaderProps> = ({
  ...
  return <header className="md:hidden flex justify-between items-center self-stretch bg-background pt-safe-header pb-2 px-3 xs:pb-2 xs:px-4 sm:px-5 sticky top-0 z-50 gap-2">
      ...
      <div className="flex items-center gap-1">
        {user && <>
            <button className="flex h-10 w-10 justify-center items-center rounded-xl hover:bg-muted/50 active:scale-95 transition-all focus-ring touch-target" aria-label="התראות" onClick={handleNotificationsClick}>
            ...
            <Button variant="ghost" size="sm" onClick={handleAuthClick} className="text-xs min-h-[44px] px-2">
              יציאה
            </Button>
          </>}
      </div>
