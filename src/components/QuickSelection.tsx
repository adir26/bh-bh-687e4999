...
export const QuickSelection: React.FC<QuickSelectionProps> = ({ items, onItemClick }) => {
  return (
    <div className="w-full mb-4 xs:mb-5 sm:mb-6">
      ...
      <div className="md:hidden flex items-start gap-4 xs:gap-6 sm:gap-8 w-full overflow-x-auto smooth-scroll scrollbar-hide px-4 xs:px-5 sm:px-6 py-3 xs:py-4">
        {items.map((item) => (
          <button
            key={item.id}
      ...
      <div className="hidden md:grid md:grid-cols-3 lg:grid-cols-5 gap-4 px-4 xs:px-5 sm:px-6 py-3 xs:py-4">
        {items.map((item) => (
          <button
            key={item.id}
            className="touch-target flex flex-col items-center gap-3 p-4 rounded-lg hover:bg-accent mobile-transition focus-ring tap-highlight-transparent no-select"
            onClick={() => onItemClick?.(item)}
            aria-label={item.title}
          >
            <div className="flex flex-col items-center w-full">
